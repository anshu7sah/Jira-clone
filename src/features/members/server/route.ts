...
const app = new Hono()
  ...
  .delete("/:memberId", sessionMiddleware, async (c) => {
    ...
    if (allMembersInWorkspace.total === 1) {
      return c.json({ error: "Can not delete only member" }, 400);
    }
  ...
  .patch(
    ...
    async (c) => {
      ...
      const allMembersInWorkspace = await databases.listDocuments(
        DATABASE_ID,
        MEMBER_ID,
        [Query.equal("worksapceId", memberToUpdate.workspaceId)]
      );
      ...
      if (allMembersInWorkspace.total === 1) {
        return c.json({ error: "Can not downgrade only member" }, 400);
      }
